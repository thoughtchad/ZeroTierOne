...
sleep 1
if [ -f /app/vendor/zerotier-one/zerotier-one.pid ]; then
	kill -TERM `cat /app/vendor/zerotier-one/zerotier-one.pid`
	sleep 1
fi
if [ -f /app/vendor/zerotier-one/zerotier-one.pid ]; then
	kill -KILL `cat /app/vendor/zerotier-one/zerotier-one.pid`
fi
...
if [ -d /app/vendor/zerotier-one ]; then
	cd /app/vendor/zerotier-one
...
rm -f /app/vendor/bin/zerotier-cli /usr/bin/zerotier-cli /app/vendor/bin/zerotier-idtool /usr/bin/zerotier-idtool

echo "Done."
echo
echo "Your ZeroTier One identity is still preserved in /app/vendor/zerotier-one"
